...
export default function suite() {
	it("Returns 401 Unauthorized, if not owned not granted", async () => {
		const response = await get(Url_GiftGetOne(GlobalVar.List2_Id, GlobalVar.Gift3_Id));
	...
	it("Returns 401 Unauthorized, if owned but gift does not belong to list", async () => {
		const response = await get(Url_GiftGetOne(GlobalVar.List1_Id, GlobalVar.Gift3_Id));
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 200 with list informations, if owned", async () => {
		const response = await get(Url_GiftGetOne(GlobalVar.List1_Id, GlobalVar.Gift1_Id));
		expect200(response);
	...
	it("Returns 422, with validation error, if path param is not UUID", async () => {
		...
		const responses = [
			await get(Url_GiftGetOne(wrongUUID, GlobalVar.Gift1_Id)),
			await get(Url_GiftGetOne(GlobalVar.List1_Id, wrongUUID)),
		];
