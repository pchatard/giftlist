...
export default function suite() {
	it("Returns 401 Unauthorized, if not owned not granted", async () => {
		const response = await put(Url_GiftHide(GlobalVar.List2_Id, GlobalVar.Gift3_Id), {});
	...
	it("Returns 401 Unauthorized, if owned but gift does not belong to list", async () => {
		const response = await put(Url_GiftHide(GlobalVar.List1_Id, GlobalVar.Gift3_Id), {});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 204, if owned", async () => {
		const response = await put(Url_GiftHide(GlobalVar.List1_Id, GlobalVar.Gift1_Id), {});
		expect204(response);
	...
	it("Returns 422, with validation error, if path param is not UUID", async () => {
		...
		const responses = [
			await put(Url_GiftHide(wrongUUID, GlobalVar.Gift2_Id)),
			await put(Url_GiftHide(GlobalVar.List1_Id, wrongUUID)),
		];
