...
import { Url_GiftGetOne, Url_GiftUnbook } from "../global";
...
export default function suite() {
	it("Returns 401 Unauthorized, if not owned not granted", async () => {
		const response = await put(Url_GiftUnbook(ListUnauthorized.id, Gift5.id), {
			title: "ChangedGift4",
		});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 204, if not owned but granted", async () => {
		const response = await put(Url_GiftUnbook(ListGranted.id, Gift3.id), {});
		expect204(response);
	...
	it("Returns 422, with validation error, if path param is not UUID", async () => {
		...
		const responses = [
			await put(Url_GiftUnbook(wrongUUID, Gift1.id)),
			await put(Url_GiftUnbook(ListOwned.id, wrongUUID)),
		];
