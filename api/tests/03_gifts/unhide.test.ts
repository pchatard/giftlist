...
export default function suite() {
	it("Returns 401 Unauthorized, if not owned not granted", async () => {
		const response = await put(Url_GiftUnhide(GlobalVar.List3_Id, GlobalVar.Gift4_Id), {
			title: "ChangedGift4",
		});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 401 Unauthorized, if not owned but granted", async () => {
		const response = await put(Url_GiftUnhide(GlobalVar.List2_Id, GlobalVar.Gift3_Id), {
			title: "ChangedGift3",
		});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 401 Unauthorized, if owned but gift does not belong to list", async () => {
		const response = await put(Url_GiftUnhide(GlobalVar.List1_Id, GlobalVar.Gift3_Id), {});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 204, if owned", async () => {
		const response = await put(Url_GiftUnhide(GlobalVar.List1_Id, GlobalVar.Gift1_Id), {});
		expect204(response);
	...
	it("Returns 422, with validation error, if path param is not UUID", async () => {
		...
		const responses = [
			await put(Url_GiftUnhide(wrongUUID, GlobalVar.Gift2_Id)),
			await put(Url_GiftUnhide(GlobalVar.List1_Id, wrongUUID)),
		];
