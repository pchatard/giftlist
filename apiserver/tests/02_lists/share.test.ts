...
import { GlobalVar, Url_ListGetOne, Url_ListShare } from "../global";
...
export default function suite() {
	it("Returns 401 Unauthorized, if not owned not granted", async () => {
		const response = await put(Url_ListShare(GlobalVar.List1_Id, GlobalVar.User2_Id), {});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 401 Unauthorized, if not owned but granted", async () => {
		const response = await put(Url_ListShare(GlobalVar.List2_Id, GlobalVar.User2_Id), {});
		expectError(response, 401, "Unauthorized");
	});
	it("Returns 204, if owned", async () => {
		const response = await put(Url_ListShare(GlobalVar.List1_Id, GlobalVar.User1_Id), {});
		expect204(response);
	...
	it("Returns 422, with validation error, if path param is not UUID", async () => {
		...
		const responses = [
			await put(Url_ListShare(wrongUUID, GlobalVar.User1_Id)),
			await put(Url_ListShare(GlobalVar.List1_Id, wrongUUID)),
		];
